...
timings_labels <- c(meltdates="Snowmelt", 
                   vt="Volatiles",
...
```{r soilmoisture}
ggplot(sm, aes(x=paste(year(date), site,date), y=VWC, color=temp)) +
  geom_boxplot()+#geom_violin(draw_quantiles=c(0.25, 0.5, 0.75)) + #geom_point()+
  theme_minimal() + theme(axis.text.x=element_text(angle=90), legend.position="top") + 
