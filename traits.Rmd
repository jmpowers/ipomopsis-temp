...
sm <- read_csv("data/2019 & 2021 SC & GNA Soil Moisture - general.csv") %>% mutate(year=year(date))
...
timings <- bind_rows(
  ...
  vt = vt %>% filter(Tissue=="flower", !is.na(site)) %>% group_by(site, year) %>% summarize(begin = min(yday(date)), end = max(yday(date))),
  mt = morph %>% drop_na(corolla_length) %>% group_by(site, year) %>% summarize(begin = min(yday(date), na.rm=T), end = max(yday(date), na.rm=T)),
