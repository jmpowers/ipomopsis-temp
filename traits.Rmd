...
flrcol%>% drop_na(redness, OI)    %>% count(site, OI, plantid) %>% count(site, OI) %>% kable(caption="redness")
...
flrcol.mean <- flrcol %>% drop_na(redness, OI) %>% 
  group_by(species, site, OI, snow, plotid, plantid) %>% 
...
ggplot(phen.mean, aes(x=paste(species, site), y=height_cm, color=OI)) + 
  ...
  scale_color_brewer(palette="Set1", labels=templabs) + theme_minimal()
ggplot(flrcol, aes(x=paste(species, site), y=redness, color=OI)) + 
  geom_violin(draw_quantiles=c(0.25, 0.5, 0.75)) + 
  geom_point(position=position_jitterdodge(dodge.width=0.9, jitter.width=0.1))+
...
lmer(height_cm   ~OI*snow+(1|plotid)+(1|plantid),data=phen) %>% anova %>% 
  kable(caption="height_cm")
lmer(redness     ~OI*snow+(1|plotid)+(1|plantid), data=flrcol) %>% anova %>% 
  kable(caption="redness")
```
...
lmer(redness       ~OI+(1|plantid), data=flrcol)       %>% anova %>% kable(caption="redness")
```
...
points(flrcol.pca, display="sites", col=flrcol$RGB, pch=c(17,19)[flrcol$OI])
...
## CAP
```{r flrcol_cap}
flrcol.cap <- capscale(t(flrspec.nm.zero.bin[,-1]) ~ OI, scale = TRUE, data=flrcol)
...
```{r flrcol_avg}
aggplot(procspec(flrspec, opt="smooth", span=0.08), flrcol$OI, legend= T, main="Raw data")
aggplot(flrspec.nm.zero, flrcol$OI, legend= T,main= "Divided by max")
aggplot(flrspec.sum.zero, flrcol$OI, legend= T, main="Divided by sum")
```
