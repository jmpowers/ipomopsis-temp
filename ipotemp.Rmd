...
svf$TotalVols <- rowSums(vol)

ipochemsl <- ipochems[match(colnames(vol.leaf), ipochems$shortname),]
vol.leaf <- sweep(vol.leaf, 2, ipochemsl$area_per_ng2019, FUN = '/')
svl$TotalVols <- rowSums(vol.leaf)

write_csv(bind_rows(floral=svf,
                    leaf=svl %>% mutate(Year = factor(Year)), .id="type"), "data/volatiles/svf_svl.csv")
```
...
lmer(sqrt(TotalVols) ~ Year + Site + temp + (1|PlantID), data = svf.otc) %>% anova %>% kable(caption="total emissions")
```
...
lmer(sqrt(TotalVols) ~ Year + temp + (1|PlantID), data = svf.otc) %>% anova %>% kable(caption="total emissions")
```
...
lmer(sqrt(TotalVols) ~ Year * DN + temp + (1|PlantID), data = svf.otc) %>% anova %>% kable(caption="total emissions")
```
...
bind_cols(svf.otc, vol.otc) %>% group_by(Site, sampround) %>% summarize(`a-pinene`=mean(`a-pinene`)) %>%
  pivot_wider(names_from=sampround, values_from=`a-pinene`) %>% mutate(ratio = `4wks`/`2wks`) %>% 
  kable(caption="a-pinene emissions")
```
...
ggplot(vol.plot %>% drop_na(PlantType, expt) %>% filter(expt=="otc", Site!="bb"),#TODO track down NAs
  ...
  facet_wrap(vars(chem), scales="free_y", labeller=as_labeller(greekify)) + 
  scale_y_sqrt("Emissions (ng/flower/hr)", expand=expansion(mult=c(0,0.05))) + 
  scale_color_brewer("Treatment", palette = "Set1", direction=-1)+
  scale_fill_manual("Time of day",values=DNcols) + 
  theme_bw() + theme(axis.text.x=element_text(angle=20, vjust=0.5), legend.position = "top", axis.text=element_text(color="black"),
...
ggplot(drop_na(vol.plot, PlantType, expt) %>% filter(expt=="otc", Site!="bb"),#TODO track down NAs
  ...
  geom_point(size=1) + geom_smooth(se=F, method="loess", span=1.2)+ 
  facet_wrap(vars(chem), scales="free_y", labeller=as_labeller(greekify)) + 
  scale_y_sqrt("Emissions (ng/flower/hr)", expand=expansion(mult=c(0,0.05))) + 
  ...
  theme_bw() + theme(legend.position = "top", axis.text=element_text(color="black"))
...
ggplot(aes(x=paste(ifelse(PlantType=="agg",Site,as.character(DN)), str_sub(Year,3,4)), y=fct_relabel(fct_rev(chem), greekify))) + 
  ...
  geom_tile(aes(fill=if_else(term =="mean_temp_C_treatment", log10(relchange+1)*5,log10(relchange+1)))) + scale_fill_gradient2(guide="none", limits=c(-1,1), oob=scales::squish, low = "orange",high="forestgreen")  + 
...
```{r cap_leaf_otc, fig.height=5, fig.width=6.5}
...
ggplot(mapping=aes(x=CAP1, y=CAP2, label=Label)) + 
  ...
  geom_text(data=cap.df %>% filter(Score=="biplot", Label %in% arrowvars[2]) %>% mutate(Label=names(arrowvars[2])),
            aes(x=CAP1*1.3, y=CAP2*1.3, angle=atan(CAP2/CAP1)*180/pi, hjust=1), fontface="bold", color="white", size=4) +
  geom_text(data=cap.df %>% filter(Score=="centroids") %>% mutate(Label=c("I. aggregata","I. tenuituba","","")),
            aes(x=CAP1, y=CAP2), fontface=4, color="white", size=4) +
  geom_text_repel(data=cap.df %>% left_join(ipochems, by=c("Label"="name")) %>% mutate(Label=greekify(Label)) %>% 
                    filter(Score=="species", sqrt(CAP1^2+CAP2^2)>0.2), 
...
lm(sqrt(TotalVols) ~ PlantType + temp, data = svf.otc) %>% anova %>% kable(caption="total emissions")
```
...
ggplot(absamounts.leaf, aes(y=fct_reorder(name, mean, base::mean), x=mean/1000, color=PlantType))+ 
  geom_pointrange(aes(xmin=mean/1000-se/1000, xmax=mean/1000+se/1000), position=position_dodge2(width=0.4)) + scale_x_sqrt(expand=c(0,0))+ 
  labs(x="Mean ng / mg dry mass / hour",y="",color="Species")
...
absamounts.leaf %>% mutate(across(all_of(c("mean", "se")), ~format(round(.x/1000, 2), nsmall = 2))) %>%
  ...
  kable(caption="Leaf volatiles, mean \U00B1 se ng / mg dry mass / hour", digits=2)
```
