...
quant.all <- read_tsv("data/volatiles/quant.tsv") %>%
  mutate(Name = recode(Name, !!!shortnames)) %>% 
  pivot_wider(names_from = "Name", values_from="Area") %>% as.data.frame()
rownames(quant.all) <- quant.all$Filename
quant.all[,1:2] <- NULL
quant.all <- quant.all[rownames(ipo.all),]#TODO 64 missing files, tho might be filtered out later anyway
...
for(compvol in colnames(quant.all)) {
  ...
  quant.slopes[compvol] = coef(lm(quant ~ qual, data=nozeros))["qual"]
...
par(mfrow=c(1,1))
quant_betterthan_qual <- c("b-myrcene", "(Z)-b-ocimene","(E)-b-ocimene", "g-terpinene","methyl benzoate", "(3E)-4,8-dimethylnona-1,3,7-triene", "2-methylbenzonitrile", "indole", "petasitene", 
 "trans-a-bergamotene","b-caryophyllene" ,"bergamotene","a-farnesene","pseudoionone")

ggplot(tibble(quant=quant.all$indole, qual=ipo.all$indole), aes(x=qual, y=quant/0.325)) + 
...
```
GNA_121 and SC_101 traps contaminated, no metadata for SC_140. maxfiles is not missing any metadata.
## Times
```{r times}
...
files_exclude <- c("T196DAug16_09012018.qgd", "A188DAug16_09012018.qgd",  "T139_day_190725_862019_06.qgd", "GNA_17_210716_7202021_18.qgd","Maxfield_27_210723_822021_03.qgd", "Maxfield_9_210723_822021_05.qgd" ,"T145DWarmAug10_08292018.qgd", #no filtered volatiles
...
#swap in quantitative integrations (scaled up to match qual integrations)
