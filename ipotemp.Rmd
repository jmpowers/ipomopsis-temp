...
meta %>% filter(is.na(Filename) & nomatch!="NR") %>% select(1:16) %>% kable(caption = "have metadata, no chromatogram")
...
#maxfiles is not missing any metadata
```
...
```{r VWCtrend, fig.width=12, fig.height=8}
ggplot(drop_na(vol.plot, PlantType, expt) %>% filter(expt=="otc", Site!="bb"),#TODO track down NAs
