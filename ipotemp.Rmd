...
ggplot(meta %>% filter(Expt=="otc", temp !="air"), 
       aes(y=mean_temp_C_treatment, x=paste(Year, Site), color=temp)) +
  facet_grid(cols=vars(DN), scales="free_x", space="free_x")+ geom_boxplot()+
  theme_minimal() + theme(axis.text.x=element_text(angle=90)) + 
  scale_color_brewer(palette = "Set1", direction=-1)+
  labs(y="Mean actual temperature (C)", x=("Year, site"), color="Temperature")

ggplot(meta %>% filter(Expt=="otc", temp !="air"), 
       aes(y=mean_temp_C_warmed, x=mean_temp_C_control, color=DN))+
  geom_abline(slope=1, intercept=0)+ geom_smooth(se=F)+
  geom_text(aes(label=round(mean_temp_C_warmed-mean_temp_C_control)))+ theme_minimal() 

ggplot(meta %>% filter(Expt=="otc", temp !="air"), 
       aes(y=mean_temp_C_warmed-mean_temp_C_control, x=hour(Pump), color=DN))+
  geom_jitter(width=0.2,height=0)+geom_smooth(se=F)+ theme_minimal() 

ggplot(meta %>% filter(Expt=="otc", temp !="air"), 
       aes(y=mean_temp_C_warmed-mean_temp_C_control, x=mean_temp_C_control, color=DN))+
  geom_point()+geom_smooth(se=F)+ theme_minimal()

ggplot(meta %>% filter(Expt=="otc", temp !="air"), 
       aes(x=hour(Pump), y=mean_temp_C_control, color=DN))+
  geom_point()+geom_smooth(aes(group=paste(Site, Year, DN)), se=F, method="lm")+ theme_minimal() +
...
plot.otc <- function(svf.otc, cap.terms=c("mean_temp_C_treatment", "temp"), plot.chems=T) {
  ...
  with(svf.otc, 
       title(paste("OTC warming:","I.",unique(PlantType), "at", paste(unique(Site), collapse=" & "),unique(Year),
                            do.call(paste0, list(unique(DN), collapse="/")))))
  if("temp" %in% cap.terms) legend("topleft", title=paste("Temperature","\nP =",anova.cap["temp","Pr(>F)"]), 
...
cap.otc18 <- plot.otc(svf %>% filter(expt=="otc",Year=="2018",PlantType=="agg"), cap.terms=c(plot.terms, "Site"))
...
cap.otc21 <- plot.otc(svf %>% filter(expt=="otc",Year=="2021",PlantType=="agg"), cap.terms=c(plot.terms, "Site"))
cap.otcSC19 <-  plot.otc(svf %>% filter(expt=="otc",Year=="2019",Site=="SC",PlantType=="ten"),cap.terms=c(plot.terms, "DN"))
cap.otcSC21 <-  plot.otc(svf %>% filter(expt=="otc",Year=="2021",Site=="SC",PlantType=="ten"),cap.terms=c(plot.terms, "DN"))
```
...
title("OTC actual temp: I. agg at all sites, all years\nvol ~ Year+Site+temp_C+temp")
temprange <- round(c(max(svf.otc$mean_temp_C_treatment), min(svf.otc$mean_temp_C_treatment)))
