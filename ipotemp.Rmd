...
ggplot(meta %>% filter(Expt=="otc", temp !="air"), aes(x=paste(Year, Site, Date), y=SoilVWC, color=temp)) +
  ...
  labs(title="Effects of OTCs on Soil Moisture at Plant",y="Soil Volumetric Water Content (%)",
...
sm.general <- read_csv("data/2019 & 2021 SC & GNA Soil Moisture - general.csv")
ggplot(sm.general, aes(x=paste(year(date), site,date), y=VWC, color=temp)) +
  geom_boxplot()+#geom_violin(draw_quantiles=c(0.25, 0.5, 0.75)) + #geom_point()+
  theme_minimal() + theme(axis.text.x=element_text(angle=90)) + 
  scale_color_brewer(palette = "Set1", direction=-1)+
  labs(title="Effects of OTCs on Soil Moisture in Center",y="Soil Volumetric Water Content (%)",
       x="Year, site, date", color="Temperature")
```
...
```{r pruning}
vol <- prune_sampletable(vol.all, chemsf_short, metadata)
...
```{r pruning_leaf}
vol.leaf <- prune_sampletable(vol.all, chemsf_short %>% mutate(filter_final=filter_final_leaf), meta.leaf)
...
vol.leaf <- vol.leaf / as.numeric(svl$Total, units="hours") / (svl$LeafDryWtGrams*1000) #area per hour per dry milligram
```
...
```{r otc_comb_year}
cap.yr.agg <- svf %>% filter(expt=="otc", PlantType=="agg", year != "2019") %>% 
  drop_na(mean_temp_C_treatment) %>% 
...
vols.plot <- chemsf_short %>% filter(filter_final, freq.floral>0.3) %>% pull("name") %>% as.character
