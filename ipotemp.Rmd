...
perc <- read_csv("data/CWu Ipomopsis Scent Metadata - Percivals2018_2019.csv")
otc <- read_csv("data/CWu Ipomopsis Scent Metadata - OTC2018_2019.csv")  %>%  
  rename(temp=OI) %>% mutate(temp = recode(temp,"in"="warmed","out"="control"))
...
ggplot(chemsf, aes(x=mean.floral, y= chemsf.leaf$mean.floral, label=name, color=paste(filter_final,chemsf.leaf$filter_final, sep=", "),
  ...
  labs(x="Mean area in flowers", y= "Mean area in leaves", color="Kept in floral, leaf datasets")
```
