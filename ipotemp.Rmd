...
#TODO CWu's Sample2 isn't always just the plant number, need to create a new plantid column
svf %>% filter(expt=="otc") %>% count(PlantType, DN, temp,Year, Site, coalesce(plantid, Sample2)) %>% 
  count(PlantType, DN, Year, Site, n)  %>%  kable(caption="Sample size per plant")
