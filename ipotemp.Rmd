...
meta %>% count(Year, Temp, Tissue, PlantType, temp, DN, Expt) %>% kable(caption="Sample size")
...
## OTC Soil moisture
```{r soil}
ggplot(meta %>% filter(Expt=="otc", temp !="air"), aes(x=paste(Year, Site, temp), y=SoilVWC, color=temp)) +
  ...
  xlab("Year, temperature")

ggplot(meta %>% filter(Expt=="otc", temp !="air"), aes(x=paste(Year, Site, temp, SampleDate_text), y=SoilVWC, color=temp)) +
  geom_boxplot()+#geom_violin(draw_quantiles=c(0.25, 0.5, 0.75)) + 
  geom_point()+
  theme_minimal() + theme(axis.text.x=element_text(angle=90)) + 
  scale_color_brewer(palette = "Set1")+
  ggtitle("Effects of OTCs on Soil Moisture")+
  ylab("Soil Volumetric Water Content (%)") +
  xlab("Year, temperature, date")
```

## OTC temperature during sampling
```{r actualtemp}
ggplot(meta %>% filter(Expt=="otc", temp !="air"), aes(y=mean_temp_C_treatment, x=paste(Year, Site, temp), color=temp)) +
...
ggplot(meta %>% filter(Expt=="otc", temp !="air", PlantType=="agg"), aes(y=mean_temp_C_treatment, x=paste(Year, Site, temp), color=temp)) +
...
# Barplots
```{r barplots, fig.width=14, fig.height=11}
