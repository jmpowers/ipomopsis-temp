...
knitr::opts_chunk$set(comment="", cache=T, warning=F, message=F, 
                      fig.path = "plots/", dev="png", fig.height=10, fig.width=10)

```

```{r read_data}
setwd("~/MyDocs/MEGA/UCI/Schiedea/Analysis/scent/rmbl/CWu/hobo")
hobo_csvs <- tibble(file=list.files(recursive=T, pattern=".csv") %>% str_remove(".csv")) %>% 
  separate(file, sep="[/-]", into=c("year","site","n1","n2","n3"), fill="right", remove = F) %>% 
  mutate(file = paste0(file, ".csv")) %>% 
  write_csv("hobo_csvs.csv")

hobometa <- read_csv("CWu OTC Hobos - metadata.csv") %>% filter(replace_na(!str_detect(notes,"drop"), TRUE))
...
hobo_objects <- map(set_names(hobometa$file), read_hobo_csv2)
...
hobos <- hobo_objects %>% map_dfr(~.x$df_env, .id="file")%>% left_join(hobometa) %>% 
  left_join(read_csv("CWu OTC Hobos - duration.csv")) %>%
